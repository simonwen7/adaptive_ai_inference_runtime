...
struct StormHarness {
    ...
    bool start(std::size_t scheduler_capacity, std::size_t worker_queue_capacity) {
        ...
        worker_config.queue_capacity = worker_queue_capacity;
        ...
        runtime = std::make_shared<Runtime>(std::make_unique<FifoScheduler>(scheduler_capacity),
...
HttpExchange http_post_infer(unsigned short port, int timeout_ms = 5000) {
    HttpExchange result;
    boost::asio::io_context ioc;
    ...
    if (status_pos != std::string::npos) {
        result.status = std::stoi(response.substr(status_pos + 1, 3));
    }
    const auto body_pos = response.find("\r\n\r\n");
    if (body_pos != std::string::npos) {
        result.body = response.substr(body_pos + 4);
    }
    result.complete = response.find("HTTP/1.1") != std::string::npos;
    return result;
}

HttpExchange http_get(unsigned short port, const std::string &path) {
    HttpExchange result;
    boost::asio::io_context ioc;
    boost::asio::ip::tcp::socket socket(ioc);
    socket.connect(
...
void run_reject_storm_once() {
    StormHarness harness;
    ASSERT_TRUE(harness.start(1, 1));

    std::thread holder([&] { (void)http_post_infer(harness.port, 30000); });
    {
    ...
    // Saturate the single worker lane slot before the reject storm.
    std::thread filler([&] { (void)http_post_infer(harness.port, 30000); });
    std::this_thread::sleep_for(std::chrono::milliseconds{50});

    std::vector<std::thread> flood;
    std::mutex results_mutex;
    std::vector<HttpExchange> results;
    for (int i = 0; i < 12; ++i) {
        flood.emplace_back([&] {
            auto exchange = http_post_infer(harness.port, 5000);
            std::lock_guard lock(results_mutex);
            results.push_back(std::move(exchange));
        });
    }

    const auto health = http_get(harness.port, "/health");
    ...
    int saw_503 = 0;
    for (const auto &exchange : results) {
        ASSERT_TRUE(exchange.complete);
        EXPECT_EQ(count_substring(exchange.body, "\"state\":\"Rejected\"") +
                      count_substring(exchange.body, "\"state\":\"Completed\""),
                  1);
        EXPECT_LE(count_substring(exchange.body, "HTTP/1.1"), 0);
...
TEST(HttpSessionTerminalResponseTest, ImmediateAdmissionQueueFullReturnsSingleTerminalResponse) {
    StormHarness harness;
    // queue_capacity=1: one active infer + one queued slot; fill both before expecting QueueFull.
    ASSERT_TRUE(harness.start(1, 1));
    ...
    // Occupy the lone worker lane slot so the next admission is rejected immediately.
    std::thread filler([&] { (void)http_post_infer(harness.port, 30000); });
    std::this_thread::sleep_for(std::chrono::milliseconds{50});

    const auto rejected = http_post_infer(harness.port, 5000);
    ASSERT_TRUE(rejected.complete);
    EXPECT_EQ(rejected.status, 503);
    EXPECT_EQ(count_substring(rejected.body, "\"state\":\"Rejected\""), 1);
    EXPECT_EQ(count_substring(rejected.body, "HTTP/1.1"), 0);
...
TEST(HttpSessionTerminalResponseTest, RejectStormHealthRemainsResponsive) {
    run_reject_storm_once();
}

TEST(HttpSessionTerminalResponseTest, RejectStormHealthRemainsResponsiveRepeated) {
    for (int i = 0; i < 100; ++i) {
        SCOPED_TRACE(i);
        run_reject_storm_once();
    }
